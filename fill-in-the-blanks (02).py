...
def play_levels(level, current_question):
    ...
    user_answer = raw_input('What should be substituted in for '
                            + '___'
    ...
    for word in temporary_text:
        if '___' + str(current_question) + '___' in word:
            word = word.replace('___'
                                + str(current_question)
                                + '___',
                                data[level]['answers'][current_question - 1])
...
def wrong_answer(level, current_question, current_attempt, max_attempts):
    ...
    if current_attempt < max_attempts - 1:
        print ('\n'
                ...
                + " trys left!")
    if current_attempt == max_attempts - 1:
        print '\n' + "That isn't the correct answer! You have only 1 try left! Make it count!"
...
def main():
    ...
    while current_attempt <= max_attempts:
        if play_levels(level, current_question):
            current_attempt = correct_answer(level,
                                             current_question,
                                             max_attempts) + 1
            current_question = current_question + 1
        else:
            current_attempt = wrong_answer(level,
                                           current_question,
                                           current_attempt,
                                           max_attempts)
